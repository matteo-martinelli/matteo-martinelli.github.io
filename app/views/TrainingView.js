...
export class TrainingView {
    ...
    getHtmlStructure() {
        return `
            <section id="course-projects"></section>
            <hr id="sections-separator">
        `;
    }

    renderCourseProjects(projects, totalStudents, totalHours) {
        ...
        intro_par.innerHTML = 
        `Since 2024, I have been providing training as a professional instructor, 
        teaching to ${total_students.outerHTML} students for more than ${total_hours.outerHTML} lesson hours.
        Below are the courses I have participated in as a trainer.`;
        ...
        for (var course_elem in projects) {
            // Getting variables
            var content = projects[course_elem];
            ...
            var course_description = this.courseDescriptionBuilder(provider, promoter, course_set_from, course_set_to, language, role, total_taught_hours.value, total_taught_hours.unit, link);
            ...
            for (var edition_elem in editions_list) {
                ...
                var edition_ul_li = document.createElement('li');
                var edition_title = this.editionTitleBuilder(ed_start_date, ed_end_date) ;
                edition_ul_li.appendChild(edition_title);
                editions_ul.appendChild(edition_ul_li);
                
                // Building and appending edition description
                var edition_description = this.editionDescriptionBuilder(ed_delivery_mode, ed_location, ed_participants_count);
                ...
                for (var module_elem in modules_list) {
                    // Fetching variables
                    ...
                    var module_description_ul = this.moduleDescriptionUnorderedListBuilder(module_topics, module_taught_hours.value, module_taught_hours.unit, module_start_date, module_end_date);
            ...
            var course_cycle_length = projects.length - 1;
    ...
    courseDescriptionBuilder(c_provider, c_promoter, from, to, lang, c_role, total_hours_value, total_hours_unit, c_link) {
        // TODO: optimize
        var course_string = ''; 
        if (c_provider != null) {
    ...
    editionTitleBuilder(ed_start, ed_end) {
    ...
    editionDescriptionBuilder(ed_delivery, ed_location, ed_participants) {
    ...
    moduleDescriptionBuilder(topics_list, hours_value, hours_unit, start_date, end_date) {
    ...
    moduleDescriptionUnorderedListBuilder(topics_list, hours_value, hours_unit, start_date, end_date) {
